...
현업의 메타데이터 불일치 문제를 정의하고 **NLP 기반 데이터 표준화 시스템**을 총괄 구축했다. 상용 거버넌스 솔루션(Databricks, Snowflake, MS Purview)을 검토했으나 모두 표준화 체계가 이미 있다는 전제의 적용 도구임을 확인하고, 표준화 체계 자체를 먼저 수립한 뒤 자동화했다. 파일럿 성공 후 전사 적용으로 전환됐고, 이후 AI 에이전트 플랫폼으로 확장된 대형 과제의 출발점이 되었다.
