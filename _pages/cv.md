...
{% include lang_toggle.liquid active='en' ko='/ko/cv/' %}
...
- **Robustness / 5-way cross-validation** — 5-fold CV showed a **671K-param BiLSTM statistically on par with the 110M KLUE** (96.18%±0.41% vs. 96.35%, p=0.73) at **1.48ms inference** (vs. 12.49ms); suffix ablation (−51%p), a RAG holdout (rejected synthetic-overfit), per-source error analysis, and a noise floor diagnosed the accuracy ceiling as a data limit.
